// Apdapted from
// https://gist.githubusercontent.com/ibireme/173517c208c7dc333ba962c1f0d67d12/raw/cad41ce129aa288e746e85337ae6e63384fa12a7/kpc_demo.c
...
static const char *kpep_config_error_names[KPEP_CONFIG_ERROR_MAX] = {"none",
                                                                     "invalid argument",
                                                                     "out of memory",
                                                                     "I/O",
                                                                     "buffer too small",
...
int bench_disable_aslr(int (*main_no_aslr)(int argc, char *argv[]), int argc, char *argv[]) {
    /* Detect if we are in the “re-execed” instance */
    if (getenv("NO_ASLR")) {
        /* Already relaunched with ASLR disabled: just run f() */
        main_no_aslr(argc, argv);
        return 0;
    }

    /* Otherwise: relaunch self with ASLR disabled */
    posix_spawnattr_t p_attr;
    posix_spawnattr_init(&p_attr);
    posix_spawnattr_setflags(&p_attr, _POSIX_SPAWN_DISABLE_ASLR);

    /* Pass an environment variable so we know in the child */
    setenv("NO_ASLR", "1", 1);

    pid_t pid;
    int status;

    int ret = posix_spawnp(&pid, argv[0], NULL, &p_attr, argv, environ);
    if (ret != 0) {
        fprintf(stderr, "posix_spawn: %s\n", strerror(ret));
        return -1;
    }

    /* Wait for child (the no-ASLR re-execed self) */
    ...
    if (WIFEXITED(status))
        ...
        fprintf(stderr, "%s terminated abnormally (status %d)\n", argv[0], status);
        return -1;
    }
}

static uint64_t ns_to_abstime(uint64_t ns) {
    mach_timebase_info_data_t tb;
    mach_timebase_info(&tb);
    // tb.numer/tb.denom gives conversion from absolute units to ns
    // We invert that: ns * denom / numer = absolute units
    __uint128_t x = (__uint128_t)ns * tb.denom;
    x /= tb.numer;
    return (uint64_t)x;
}

static int set_realtime_thread(uint64_t period_ns, uint64_t computation_ns, uint64_t constraint_ns,
                               boolean_t preemptible) {
    thread_port_t thr = pthread_mach_thread_np(pthread_self());

    thread_time_constraint_policy_data_t policy = {0};
    policy.period = ns_to_abstime(period_ns);
    policy.computation = ns_to_abstime(computation_ns);
    policy.constraint = ns_to_abstime(constraint_ns);
    policy.preemptible = preemptible;

    kern_return_t kr = thread_policy_set(thr, THREAD_TIME_CONSTRAINT_POLICY, (thread_policy_t)&policy,
                                         THREAD_TIME_CONSTRAINT_POLICY_COUNT);
    if (kr != KERN_SUCCESS) {
        fprintf(stderr, "thread_policy_set failed: %s (0x%x)\n", mach_error_string(kr), kr);
        return -1;
    }
    return 0;
}
...
int bench_init(void) {
    ...
    // Ensure code runs in performance cores -- possibly overriden by the real-time priority request below, but I guess
    // it doesn't hurt to leave it here
    pthread_set_qos_class_self_np(QOS_CLASS_USER_INTERACTIVE, 0);

    // Switch to real-time priority
    set_realtime_thread(RT_NS, RT_NS, RT_NS, TRUE);
...
u64 bench_get_cycles(counter_t counter_start, counter_t counter_end) {
    usize idx = counter_map[0];
    return counter_end[idx] - counter_start[idx];
}

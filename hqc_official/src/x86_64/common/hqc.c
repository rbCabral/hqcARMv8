...
void hqc_pke_keygen(uint8_t *ek_pke, uint8_t *dk_pke, uint8_t *seed) {
    ...
    static __m256i h[VEC_N_256_NUM_WORDS];
    static __m256i y[VEC_N_256_NUM_WORDS];
    static __m256i x[VEC_N_256_NUM_WORDS];
    static uint64_t s[VEC_N_256_SIZE_64] = {0};
    static __m256i tmp[VEC_N_256_NUM_WORDS] = {0};

#ifdef __STDC_LIB_EXT1__
    memset_s(x, 0, (VEC_N_256_NUM_WORDS) * sizeof(__m256i));
    memset_s(y, 0, (VEC_N_256_NUM_WORDS) * sizeof(__m256i));
    memset_s(h, 0, (VEC_N_256_NUM_WORDS) * sizeof(__m256i));
#else
    memset(x, 0, (VEC_N_256_NUM_WORDS) * sizeof(__m256i));
    memset(y, 0, (VEC_N_256_NUM_WORDS) * sizeof(__m256i));
    memset(h, 0, (VEC_N_256_NUM_WORDS) * sizeof(__m256i));
#endif
    ...
    vect_set_random(&ek_xof_ctx, (uint64_t *)h);
    vect_mul(tmp, y, h);
    vect_add(s, (uint64_t *)x, (uint64_t *)tmp, VEC_N_256_SIZE_64);
    ...
    printf("\n\ny: ");
    vect_print((uint64_t *)y, VEC_N_SIZE_BYTES);
    printf("\n\nx: ");
    vect_print((uint64_t *)x, VEC_N_SIZE_BYTES);
    printf("\n\nh: ");
    vect_print((uint64_t *)h, VEC_N_SIZE_BYTES);
    printf("\n\ns: ");
...
void hqc_pke_encrypt(ciphertext_pke_t *c_pke, const uint8_t *ek_pke, const uint64_t *m, const uint8_t *theta) {
    ...
    static __m256i h[VEC_N_256_NUM_WORDS];
    static __m256i s[VEC_N_256_NUM_WORDS];
    static __m256i r2[VEC_N_256_NUM_WORDS];
...
#ifdef __STDC_LIB_EXT1__
    memset_s(r2, 0, (VEC_N_256_NUM_WORDS) * sizeof(__m256i));
    memset_s(h, 0, (VEC_N_256_NUM_WORDS) * sizeof(__m256i));
    memset_s(s, 0, (VEC_N_256_NUM_WORDS) * sizeof(__m256i));
    memset_s(r1, 0, (VEC_N_256_NUM_WORDS) * sizeof(__m256i));
    memset_s(e, 0, (VEC_N_256_NUM_WORDS) * sizeof(__m256i));
#else
    memset(r2, 0, (VEC_N_256_NUM_WORDS) * sizeof(__m256i));
    memset(h, 0, (VEC_N_256_NUM_WORDS) * sizeof(__m256i));
    memset(s, 0, (VEC_N_256_NUM_WORDS) * sizeof(__m256i));
    ...
    hqc_ek_pke_from_string((uint64_t *)h, (uint64_t *)s, ek_pke);
    ...
    // Compute u = r1 + r2.h
    vect_mul(tmp1, r2, h);
    vect_add(c_pke->u, (uint64_t *)r1, (uint64_t *)tmp1, VEC_N_256_SIZE_64);
    ...
    vect_mul(tmp2, r2, s);
    vect_add(tmp3, (uint64_t *)e, (uint64_t *)tmp2, VEC_N_256_SIZE_64);
    vect_truncate(tmp3);
    ...
    printf("\n\nr1: ");
    vect_print((uint64_t *)r1, VEC_N_SIZE_BYTES);
    printf("\n\nr2: ");
    vect_print((uint64_t *)r2, VEC_N_SIZE_BYTES);
    printf("\n\ne: ");
    vect_print((uint64_t *)e, VEC_N_SIZE_BYTES);
    printf("\n\nTruncate(s.r2 + e): ");
    vect_print(tmp3, VEC_N1N2_SIZE_BYTES);
    printf("\n\nc_pke->u: ");
...
uint8_t hqc_pke_decrypt(uint64_t *m, const uint8_t *dk_pke, const ciphertext_pke_t *c_pke) {
    ...
    vect_mul(tmp1, y, u);
    // Truncate(u.y)
    vect_truncate((uint64_t *)tmp1);
    // Compute v - Truncate(u.y)
    vect_add(tmp2, c_pke->v, (uint64_t *)tmp1, VEC_N1N2_SIZE_64);

#ifdef VERBOSE
    printf("\n\nc_pke.u: ");
    vect_print((uint64_t *)c_pke->u, VEC_N_SIZE_BYTES);
    printf("\n\nc_pke.v: ");
    vect_print(c_pke->v, VEC_N1N2_SIZE_BYTES);
    printf("\n\ny: ");
    vect_print((uint64_t *)y, VEC_N_SIZE_BYTES);
    printf("\n\nTruncate(u.y): ");
    vect_print((uint64_t *)tmp1, VEC_N1N2_SIZE_BYTES);
    printf("\n\nv - Truncate(u.y): ");

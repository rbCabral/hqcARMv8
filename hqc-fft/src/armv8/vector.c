...
void vect_write_support_to_vector(uint64_t *v, uint32_t *support, uint16_t weight) {
    ...
    uint64x2_t bit256[PARAM_OMEGA_R << 1];
    uint64x2_t bloc256[PARAM_OMEGA_R];
    static uint64x2_t posCmp1 = {0UL,1UL};
    static uint64x2_t posCmp2 = {2UL,3UL};

    for (uint32_t i = 0 ; i < weight ; i++) {
        // we store the bloc number and bit position of each vb[i]
        uint64_t bloc = support[i] >> 6;
        bloc256[i] = vdupq_n_u64(bloc >> 2);
        uint64_t pos = (bloc & 0x3UL);
        uint64x2_t pos256 = vdupq_n_u64(pos);
        uint64x2_t mask1 = vceqq_u64(pos256, posCmp1);
        uint64x2_t mask2 = vceqq_u64(pos256, posCmp2);
        uint64_t bit64 = 1ULL << (support[i] & 0x3f);
        uint64x2_t bl256 = vdupq_n_u64(bit64);
        bit256[(i << 1)] = vandq_u64(bl256, mask1);
        bit256[(i << 1) + 1] = vandq_u64(bl256, mask2);
    }

    for (uint32_t i = 0 ; i < LOOP_SIZE ; i++) {
        uint64x2_t aux1 = {0,0};
        uint64x2_t aux2 = {0,0};
        uint64x2_t i256 = vdupq_n_u64(i);

        for (uint32_t j = 0 ; j < weight ; j++) {
            uint64x2_t mask = vceqq_u64(bloc256[j], i256);
            aux1 = veorq_u64(aux1, vandq_u64(bit256[(j << 1)], mask)) ;
            aux2 = veorq_u64(aux2, vandq_u64(bit256[(j << 1) + 1], mask)) ;
        }
        v[i << 2] ^= vgetq_lane_p64(aux1, 0);
        v[(i << 2) + 1] ^= vgetq_lane_p64(aux1, 1);
        v[(i << 2) + 2] ^= vgetq_lane_p64(aux2, 0);
        v[(i << 2) + 3] ^= vgetq_lane_p64(aux2, 1);
    }

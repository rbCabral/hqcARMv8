...
void hqc_pke_encrypt(ciphertext_pke_t *c_pke, const uint8_t *ek_pke, const uint64_t *m, const uint8_t *theta) {
    ...
    ring_mul_x2( (uint64_t*)tmp1, (uint64_t*)tmp2, (uint64_t*)r2, (uint64_t*)h, (uint64_t*)s );
    vect_add(c_pke->u, (uint64_t *)r1, (uint64_t *)tmp1, VEC_N_SIZE_64);
    ...
    vect_add(tmp3, (uint64_t *)e, (uint64_t *)tmp2, VEC_N_SIZE_64);
    vect_truncate(tmp3);
...
uint8_t hqc_pke_decrypt(uint64_t *m, const uint8_t *dk_pke, const ciphertext_pke_t *c_pke) {
    ...
    ring_mul((uint64_t *)tmp1, (uint64_t *)c_pke->u, (uint64_t *)y);
    // Truncate(u.y)
    vect_truncate(tmp1);

 ...
 **/
static const __m256i param256[2] = {{0x0074009900450059, 0x004B006F007500B0, 0x00E900F200E90049, 0x008B001500D20041},
...
void reed_solomon_encode(uint64_t *cdw, const uint64_t *msg) {
    ...
    // hqc-1 k=16
    __m256i multab[16];
    gf256v_generate_multabs_avx2((uint8_t *)multab, (uint8_t *)msg, 16);
...
void compute_syndromes(__m256i *syndromes256, uint8_t *cdw) {
    ...
    for( int i = 0; i < PARAM_N1-1; i ++)
    {
        ...
        last_syndromes256 ^= linearmap_8x8_ymm(alpha_ij256_2[i], ml, mh, mask);
    }
